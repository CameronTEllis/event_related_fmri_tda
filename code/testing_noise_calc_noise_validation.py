...
root = './'
original_path = root + 'simulator_parameters/noise_dict/'
validated_path = root + 'simulated_data/resampled_noise_dicts/'
output_path = root + 'validation_summary/'
...
def run_test(original_path, validated_path, output_path, ppt):

    # Convert inputs to outputs
    real_noise_dict_name = original_path + ppt + '.txt'
    simulated_noise_dict_root = validated_path + ppt
    output_root = output_path + ppt
...
runs = 5
ppts = 20
real_all = np.zeros((ppts, runs, noise_types))
sim_all = np.zeros((ppts, runs, noise_types, 10))
means_all = np.zeros((ppts, runs, noise_types))
stds_all = np.zeros((ppts, runs, noise_types))
for ppt in list(range(1, ppts + 1)):
    for run in list(range(1, runs + 1)):

        ppt_name = 'sub-' + str(ppt) + '_r' + str(run)
    
        # Don't match noise
        real, sim = run_test(original_path, validated_path, output_path, ppt_name)
...
np.save(output_path + 'real_noise_dicts.npy', real_all)
np.save(output_path + 'simulated_noise_dicts.npy', sim_all)
np.save(output_path + 'simulated_noise_dicts_means.npy', means_all)
np.save(output_path + 'simulated_noise_dicts_stds.npy', stds_all)
